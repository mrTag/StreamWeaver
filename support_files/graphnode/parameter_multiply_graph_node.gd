@tool
class_name ParameterMultiplyGraphNode extends GraphNode

const LEFT_PORT_INPUT_PARAMETERS : int = 0

var current_resource: StreamWeaverParameterMultiply

func initialize(res: StreamWeaverParameterMultiply):
	current_resource = res
	_update_ui()

static func create_new_resource(audio_stream_resource: StreamWeaverAudioStream) -> StreamWeaverParameterMultiply:
	var res := StreamWeaverParameterMultiply.new()
	res.resource_name = "Parameter Multiply %d" % (audio_stream_resource.parameters.size() + 1)
	return res
